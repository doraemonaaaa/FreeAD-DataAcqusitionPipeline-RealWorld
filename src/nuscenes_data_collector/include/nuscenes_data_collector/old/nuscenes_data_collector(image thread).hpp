...
private:
    ...
    void capture_image_callback(){
        //if ((!latest_ego_pose_.empty()) && (!latest_imu_.empty())) {
            // 启动异步任务并将其存储在 img_read_futures_ 中
            for (const auto &camera_name : camera_names_)
            {
                auto future = std::async(std::launch::async, &NuScenesDataCollector::image_save_async, this, camera_name);
                std::lock_guard<std::mutex> lock(futures_mutex_);
                img_read_futures_[camera_name] = std::move(future);
            }

            // 启动一个线程，用于检查任务完成状态
            check_img_read_status_thread_ = std::thread(&NuScenesDataCollector::check_async_task_status, this);
        //}
    }

     // 检查所有异步任务是否完成
    void check_async_task_status()
    {
        try {
            ...
            {
                std::lock_guard<std::mutex> lock(futures_mutex_);
                for (auto it = img_read_futures_.begin(); it != img_read_futures_.end();)
                {
                    if (it->second.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready)
                    {
                        // 任务已完成，获取结果并移除任务
                        it->second.get();  // 获取异步任务结果
                        completed_keys.push_back(it->first);  // 记录已完成任务的键
                        it = img_read_futures_.erase(it);  // 从列表中移除已完成的任务
                    }
                    else
                    {
                        all_completed = false;  // 只要有一个未完成，标记为未完成
                        ++it;
                    }
                }
            }
            // 如果超过100ms仍未完成，清空所有任务
            if (!all_completed && !completed_keys.empty())
            {
                std::this_thread::sleep_for(std::chrono::milliseconds(100));  // 等待100ms检查
                std::lock_guard<std::mutex> lock(futures_mutex_);
                img_read_futures_.clear();
                RCLCPP_WARN(this->get_logger(), "Some tasks were not completed in time. Cleared all futures.");
            }
        } 
        catch (const std::exception& e) {
            RCLCPP_ERROR(this->get_logger(), "Error in check_async_task_status: %s", e.what());
        }
    }

    void image_save_async(const std::string &camera_name)
    {
        ...
        if (success)
        {
            ...
            << camera_name << "__" << timestamp.seconds() << "_" 
            << timestamp.nanoseconds() << ".jpg";
            ...
            if (!encode_success)
            {
                ...
                return; // Skip this image and move to the next one
            }
            ...
            // Prepare JSON metadata for the image (sample_data format)
            ...
            sample_data_json["timestamp"] = timestamp.seconds() * 1e9 + timestamp.nanoseconds(); // Full timestamp in nanoseconds
            sample_data_json["filename"] = extractRelativePath(image_save_file);  // Extract relative path
            sample_data_json["fileformat"] = "jpg";  // JPEG format
            sample_data_json["height"] = image.rows;
            sample_data_json["width"] = image.cols;
            sample_data_json["is_key_frame"] = false;  // Example: can be dynamic if needed
            sample_data_json["calibrated_sensor_token"] = get_calibration_token(calibrated_sensor_token_map_, camera_name);
            ...
            // Add the sample data to the JSON writer
            json_writer_.addKeyValuePairToJson("sample_data", sample_data_json, true);

            // Update the frame count
            devices_frame_count_[camera_name]++;

            // Optional logging
            RCLCPP_INFO(this->get_logger(), "Captured image from %s at timestamp %f", camera_name.c_str(), timestamp.seconds());
    ...
    std::unordered_map<std::string, std::future<void>> img_read_futures_;  // 用于存储异步任务的 future 对象
    std::mutex futures_mutex_;  // 用于保护 futures 数据的互斥锁
    std::thread check_img_read_status_thread_;
